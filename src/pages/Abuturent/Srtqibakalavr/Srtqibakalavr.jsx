...
const Srtqibakalavr = () => {   
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    ...
    http.post("/api/user/graduatesSave" , {
        hashId:arr?.at(-1),
        informationEN: descriptionen,
        informationKR: descriptionkr,
        informationRU: descriptionru,
...
const getData =()=>{
  http.get("/api/public/partTimeUndergraduate" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/graduatesDelete/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>Srtqi kvotalari </h2>
