...
const Vakansiyalar = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editVacancie/${editid}` , {
      nameKR: titleKr,
      nameRU: titleRu,
      nameUZ: titleUz,
      nameEN: titleEn,
    ...
    http.post("/api/user/addVacancie" , {
        nameKR: titleKr,
        nameRU: titleRu,
        nameUZ: titleUz,
        nameEN: titleEn,
...
const getData =()=>{
  http.get("/api/public/allVacancie" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteVacancie/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (
    ...
    <VakansiyaTable handleEdit={handleEdit} data={allData} handleDelete={deleteData}/>
    ...
    >
      <Box  sx={style}>
        ...
        <div>
        <TextField style={{marginTop:"20px" , width:"100%" , marginBottom:"10px"}} defaultValue={salary} onChange={(e) => setSalary(e.target.value)} id="outlined-basic" label="Salary" variant="outlined" />
        </div>         
