...
const Meyoriyhujjat = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editRegulationdocument/${editid}` , {
      hashId:arr?.at(-1),
      nameKR: titleKr,
      nameRU: titleRu,
      nameUZ: titleUz,
    ...
    http.post("/api/user/addRegulationdocument" , {
        hashId:arr?.at(-1),
        nameKR: titleKr,
        nameRU: titleRu,
        nameUZ: titleUz,
...
const getData =()=>{
  http.get("/api/public/allRegulationdocument" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteRegulationdocument/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (
    ...
    <Nizomtable handleEdit={handleEdit} data={allData} handleDelete={deleteData}/>
    <Modal
      open={modal}  
      onClose={()=>handleClose()}
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>Me'yoriy hujjat qushish</h2>
