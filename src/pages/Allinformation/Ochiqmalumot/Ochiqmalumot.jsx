...
import OpendataTable from '../../../components/Allinfor/OpendataTable';
...
const Ochiqmalumot = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editOpendata/${editid}` , {
      hashId:arr?.at(-1),
      nameKR: titleKr,
      nameRU: titleRu,
      nameUZ: titleUz,
    ...
    http.post("/api/user/addOpendata" , {
        hashId:arr?.at(-1),
        nameKR: titleKr,
        nameRU: titleRu,
        nameUZ: titleUz,
...
const getData =()=>{
  http.get("/api/public/allOpendata" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteOpendata/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (
    ...
    <OpendataTable handleEdit={handleEdit} data={allData} handleDelete={deleteData}/>
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>Ochiq ma'lumotlar</h2>
