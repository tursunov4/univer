...
const Filialnizom = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editRegulation/${editid}` , {
      hashId:arr?.at(-1),
      nameKR: titleKr,
      nameRU: titleRu,
      nameUZ: titleUz,
    ...
    http.post("/api/user/addRegulation" , {
        hashId:arr?.at(-1),
        nameKR: titleKr,
        nameRU: titleRu,
        nameUZ: titleUz,
...
const getData =()=>{
  http.get("api/public/allRegulation" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteRegulation/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>Filial nizomi qushish</h2>
