...
const Kengashlar = () => {
  ...
  const handleSubmit = () => {
    ...
    if (edit) {
      http
        .put(`/api/user/editCouncils/${editid}`, {
          hashid: arr?.at(-1),
          boardNameKR: titleKr,
          boardNameRU: titleRu,
          boardNameUZ: titleUz,
  ...
  const deleteData = (id) => {
    http
      .delete(`/api/user/deleteCouncils/${id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data?.success) {
      ...
      >
        <Box sx={style}>
          <h2 className="elontitle">
            {" "}
            Kengash {edit ? "tahrirlash" : "qo'shish"}
          </h2>
