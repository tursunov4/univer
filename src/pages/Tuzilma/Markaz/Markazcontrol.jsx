...
const Markazcontrol = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editCenterAndDepartment/${editid}` , {
    ...
    http.post("/api/user/addCenterAndDepartment" , {
      descriptionEN: descriptionen,
      descriptionKR: descriptionkr,
      descriptionRU: descriptionru,
      descriptionUZ: descriptionuz,
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteCenterAndDepartment/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (  
    ...
    <MarkazcontrolTable handleEdit={handleEdit} data={allData} handleDelete={deleteData}/>
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>Markaz bo'lim  {edit ? "tahrirlash" : "qo'shish"}</h2>
