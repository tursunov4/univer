...
const Interaktivservive = () => {
  ...
  const handleSubmit = () => {
    ...
    if (edit) {
      http
        .put(`/api/user/editservice/${editid}`, {
          icon: arr?.at(-1),
  ...
  const deleteData = (id) => {
    http
      .delete(`/api/user/deleteservice/${id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data?.success) {
        ...
        <Box sx={style}>
          ...
          <div className="elonimg">
            <label className="eloninput">
              ...
              <span>
                Faqat bitta icon qushish mumkin . Icon qo'shish majburiy !!!
              </span>
            </label>
          </div>
