...
const Aboutus = () => {
  ...
  const delateImage = (item) => {
    http
      .delete(`/api/user/delete/${item.hashId}`)
      .then((res) => {
        console.log(res.data);
        const indexToDelete = imagearr.findIndex(
  ...
  const handleSubmit = () => {
    ...
    if (edit) {
      http
        .put(`/api/user/editAboutInstituti/${editid}`, {
  ...
  const getData = () => {
    http
      .get("/api/public/getAboutInstitutiall")
      .then((res) => {
        console.log(res.data);
        setAlldata(res.data);
      })
  ...
  const deleteData = (id) => {
    http
      .delete(`/api/admin/deleteAboutInstituti/${id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data?.success) {
  ...
  return (
    <>
      ...
      >
        <Box sx={style}>
          <h2 className="elontitle">Biz haqimizda</h2>
          ...
          <div className="elonimg">
            <label className="eloninput">
              ...
              <span>Maxsimum 10 ta image gacha mumkin</span>
            </label>
          </div>
          <div className="elonimge__names">
            {imagearr?.map((item, index) => (
              <div key={index} className="elonimage__inner">
                {item?.orginalName}
                <CloseIcon
                  onClick={() => delateImage(item)}
                  className="closeinner"
                />
              </div>
            ))}
          </div>
