...
const Newsday = () => {
  ...
  const handleSubmit = () => {
    ...
    if (edit) {
      http
        .put(`/api/user/editNewsDay/${editid}`, {
          hashId: arr?.at(-1),
  ...
  const deleteData = (id) => {
    http
      .delete(`/api/user/deleteNewsDay/${id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data?.success) {
        ...
        <Box sx={style}>
          ...
          <div className="elonimg">
            <label className="eloninput">
              ...
              <span>Faqat bitta image qushish mumkin</span>
            </label>
          </div>
