...
const Ilmiykengash = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editScientificActivities/${editid}` , {
      hashId:arr?.at(-1),
      ...
      nameUZ: titleUz,
      nameEN: titleEn,
      scientificActivitiesStatus: "ILMIY_KENGASH"
    }).then((res) =>{
      console.log(res.data)
      if(res.data?.success){
    ...
    http.post("/api/user/addScientificActivities" , {
        hashId:arr?.at(-1),
        informationEN: descriptionen,
        informationKR: descriptionkr,
        informationRU: descriptionru,
...
const getData =()=>{
  http.get("/api/public/allScientificCouncils" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteScientificActivities/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (  
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>Ilmiy kengash</h2>
