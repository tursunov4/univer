...
const Uquvgrafig = () => {
  ...
  const handleSubmit = () => {
    ...
    if (edit) {
      http
        .put(`/api/user/editEducationalActivities/${editid}`, {
          hashId: arr?.at(-1),
      ...
      http
        .post("/api/user/addEducationalActivities", {
          hashId: arr?.at(-1),
          informationEN: descriptionen,
          informationKR: descriptionkr,
          informationRU: descriptionru,
  ...
  const deleteData = (id) => {
    http
      .delete(`/api/user/deleteEducationalActivities/${id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data?.success) {
