...
const Uquvgrafig = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editEducationalActivities/${editid}` , {
      hashId:arr?.at(-1),
      ...
      nameUZ: titleUz,
      nameEN: titleEn,
      educationalActivitiesStatus: "UQUV_GRAFIGI"
    }).then((res) =>{
      console.log(res.data)
      if(res.data?.success){
    ...
    http.post("/api/user/addEducationalActivities" , {
        hashId:arr?.at(-1),
        informationEN: descriptionen,
        informationKR: descriptionkr,
        informationRU: descriptionru,
...
const getData =()=>{
  http.get("/api/public/allStudyCharts" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteEducationalActivities/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (  
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>O'quv grafig</h2>
