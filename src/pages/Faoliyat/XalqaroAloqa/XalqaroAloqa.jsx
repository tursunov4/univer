...
const XalqaroAloqa = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editInternationalRelations/${editid}` , {
      hashId:arr?.at(-1),
      nameKR: titleKr,
      nameRU: titleRu,
      nameUZ: titleUz,
    ...
    http.post("/api/user/addInternationalRelations" , {
        hashId:arr?.at(-1),
        nameKR: titleKr,
        nameRU: titleRu,
        nameUZ: titleUz,
...
const getData =()=>{
  http.get("/api/public/allInternationalRelations" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteInternationalRelations/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (
    ...
    >
      <Box  sx={style}>
         <h2 className='elontitle'>Xalqaro aloqalar</h2>
