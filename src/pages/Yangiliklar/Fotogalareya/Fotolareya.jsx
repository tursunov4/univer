...
const Fotogalareya = () => {
 ...
 const handleSubmit =()=>{
  ...
  if(edit){
    http.put(`/api/user/editFile/${editid}` , {
      hashIds:arr,
      titleKR: titleKr,
      titleRU: titleRu,
      titleUZ: titleUz,
...
const getData =()=>{
  http.get("/api/public/allFoto" ).then((res) =>{
    console.log(res.data)
    setAlldata(res.data)
  }).catch((err)=>{
    console.log(err)
...
const deleteData =(id)=>{
  http.delete(`/api/user/deleteFoto/${id}`).then((res) =>{
    console.log(res.data)
    if(res.data?.success){
...
useEffect(()=>{
  ...
  return (
    ...
    <PhotoTable handleEdit={handleEdit} data={allData} handleDelete={deleteData}/>
