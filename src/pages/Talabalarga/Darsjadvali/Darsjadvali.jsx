...
const Darsjadvali = () => {
  ...
  const handleSubmit = () => {
    ...
    if (edit) {
      ...
      http
        .post("/api/user/classSchedulesSave", {
          hashId: arr?.at(-1),
          informationEN: descriptionen,
          informationKR: descriptionkr,
          informationRU: descriptionru,
  ...
  const getData = () => {
    http
      .get("/api/public/allclassSchedules")
      .then((res) => {
        console.log(res.data);
        setAlldata(res.data);
      })
  ...
  const deleteData = (id) => {
    http
      .delete(`/api/user/classSchedulesDelete/${id}`)
      .then((res) => {
        console.log(res.data);
        if (res.data?.success) {
  ...
  return (
    <>
      ...
      >
        <Box sx={style}>
          <h2 className="elontitle">Dars Jadvali</h2>
